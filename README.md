...
5. 번역 결과를 `{id: 번역문}` 형태로 JS에 다시 넘기면, JS가 저장해둔 블록 요소
   ...
   요소가 **있으면** 그 구조를 지우지 않도록 첫 텍스트 노드에만 번역문 전체를
   넣고 나머지 텍스트 노드는 비운다(대가로 번역문이 블록 안의 원래 텍스트 배치와
   똑같이 나뉘어 보이지는 않는다). 이 구분이 없던 예전 버전에서는 "다음 화" 링크가
   문단 텍스트 안에 있는 사이트에서 번역 후 그 링크 자체가 사라져 클릭이 안 되는
   문제가 있었다.
...
- 인터랙티브 요소가 있는 블록에서 번역문을 첫 텍스트 노드에 몰아넣는 대신,
  원문의 텍스트 배치 비율에 맞게 여러 텍스트 노드에 나눠 넣는 정교한 방식
  (현재는 클릭 가능 여부만 보장하고 텍스트 배치의 정교함은 희생함)
- 최근 방문 URL 히스토리/즐겨찾기
