...
- **출발어 드롭다운의 "자동 감지" — 다국어 혼합 페이지 지원** — 다국어
  ...
  문단은 판별 후 그대로 둔다. 문단마다 판별이 새로 필요해 일반 언어 지정보다는
  다소 느릴 수 있고, 같은 페이지를 재방문해도 번역 캐시를 쓰지 않는다.
...
- **최근 방문 / 즐겨찾기** — "기록" 버튼에서 최근 방문한 URL 목록과 즐겨찾기를 보고
  바로 이동할 수 있다.
- **사용량 확인** — "기록" 버튼을 길게 누르면 이번 달 Cloud Translation(글자 수)/
  LLM 후처리 사용량을 볼 수 있다. LLM 쪽은 API 응답에 실제 토큰 수(`usage` 필드)가
  있으면 그 값을, 없으면 글자 수 근사치를 쓰므로 "토큰/글자 추정"이라고 표시한다.
  둘 다 선택 설정이라 아무 키도 넣지 않았다면 항상 0.
