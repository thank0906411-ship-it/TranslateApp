...
- **출발어 선택 & 페이지 언어 자동 반영** — 상단 드롭다운에서 출발어를 직접 고를
  수 있고(기본값 영어), 페이지에 `<html lang="...">`이 있으면 자동으로 맞춰준다.
  드롭다운을 한 번이라도 직접 조작하면 그 뒤로는 이 자동 반영보다 사용자의
  선택을 우선한다. 지원 언어: 한국어/영어/일본어/중국어(간체)/프랑스어/독일어/
  스페인어/러시아어/베트남어/태국어 (`translation/SupportedLanguages.kt`에 추가 가능).
  도착어는 항상 한국어로 고정되어 있어(다른 언어로 번역할 일이 거의 없어 드롭다운
  자체를 없앰) 별도로 선택할 필요가 없다. 도착어를 바꾸고 싶으면
  `SupportedLanguages.DEFAULT_TARGET`을 수정하고 다시 빌드해야 한다.
- **출발어 드롭다운의 "자동 감지" — 다국어 혼합 페이지 지원** — 다국어
  커뮤니티/포럼처럼 한 페이지 안에 여러 언어가 섞여 있으면, 출발어를 하나로
  고정해서는 그 언어가 아닌 문단이 번역되지 않는다. 드롭다운 맨 위 "자동 감지"를
  선택하면 문단마다 실제 언어를 판별해 그 언어에서 번역하고, 이미 한국어인
  문단은 판별 후 그대로 둔다. 처음 만나는 문장은 일반 언어 지정보다 판별 과정이
  하나 더 있어 다소 느릴 수 있지만, 판별 결과 자체를 캐싱해두므로(도착어가
  바뀌어도 재사용됨) 같은 문장을 다시 만나면 판별을 건너뛰고 번역 캐시까지
  그대로 재사용한다.
- **용어 일관성(용어집)** — 상단 "설정"(톱니바퀴) 버튼 → "용어집"에서 원문 용어 →
  고정 번역어 쌍을 등록해두면, 그 용어가 나올 때마다 항상 같은 번역어로 표시된다.
  대소문자는 구분하지 않는다. 다이얼로그의 "내보내기"/"가져오기" 버튼으로 JSON
  파일을 공유하거나 불러올 수 있어, 기기 변경/재설치 시에도 용어집을 백업·복원할
  수 있다(가져오기는 기존 용어집을 지우지 않고 병합).
- **최근 방문 / 즐겨찾기** — "기록" 버튼에서 최근 방문한 URL 목록과 즐겨찾기를 보고
  바로 이동할 수 있다.
- **사용량 확인** — 상단 "설정"(톱니바퀴) 버튼 → "사용량"에서 이번 달 Cloud
  Translation(글자 수)/LLM 후처리 사용량을 볼 수 있다(기존처럼 "기록" 버튼을
  길게 눌러도 같은 정보가 Toast로 뜬다). LLM 쪽은 API 응답에 실제 토큰 수
  (`usage` 필드)가 있으면 그 값을, 없으면 글자 수 근사치를 쓰므로 "토큰/글자
  추정"이라고 표시한다. 둘 다 선택 설정이라 아무 키도 넣지 않았다면 항상 0.
- **번역 실패 표시 및 재시도** — 번역이 제대로 되지 않은 문장은 점선 밑줄로 표시되고,
  페이지당 한 번 안내를 띄운다. 링크/버튼 등이 없는 블록은 눌러서 그 블록만 다시
  번역을 시도할 수 있다(인터랙티브 요소가 있는 블록은 클릭이 링크 이동과 겹칠 수
  있어 재시도 탭 대신 안내 문구만 표시).
- **원문 대조 보기** — 번역된 문단을 길게 누르면(0.5초) 원문을 Toast로 보여준다.
  번역이 이상하게 됐는지 확인하고 싶을 때 유용하다(링크/버튼이 있는 블록은
  롱프레스가 브라우저 컨텍스트 메뉴와 겹칠 수 있어 제외).
...
**앱 안에서 직접 설정(권장, 누구나 가능)** — 화면 상단 "설정"(톱니바퀴) 버튼 →
"LLM 설정"에서 Claude/GPT
...
전송되지 않는다(`settings/ApiKeyStore.kt`, `SharedPreferences`). "저장"을 누르면
입력한 키로 최소 비용 테스트 호출을 보내 실제로 유효한지 먼저 확인한 뒤 저장한다 —
오타나 잘못된 키를 그냥 저장해두면 나중에 번역할 때 조용히 1차 번역으로 폴백되어
원인을 알기 어렵기 때문이다.
...
첨부되는 APK 파일명도 `app-test-v3-beta1.apk`처럼 파일명 자체에 테스트용임을
명시한다(정식 배포는 `app-release.apk`) — 다운로드한 파일만 보고도 실수로 정식
배포용으로 착각해 퍼뜨리는 일을 막기 위해서다.
