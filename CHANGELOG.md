...
- **설정 메뉴에 "사용량" 항목 추가**: 지금까지 이번 달 Cloud Translation/LLM
  사용량은 "기록 버튼 길게 누르기"라는, 문서를 안 읽으면 알기 어려운 숨겨진
  제스처로만 볼 수 있었다. 상단 "설정"(톱니바퀴) 메뉴에 "사용량" 항목을
  추가해 다이얼로그로 같은 정보를 보여준다. 기존 롱프레스 Toast는 이미 아는
  사용자를 위해 그대로 유지(하위 호환).

## 새 기능 (용어집 가져오기/내보내기)

- **용어집을 JSON으로 내보내기/가져오기**: 기기 변경이나 재설치 시 등록해둔
  용어집이 전부 날아가는 문제가 있었다. 용어집 다이얼로그에 "내보내기"/
  "가져오기" 버튼을 추가했다. 내보내기는 `Glossary.exportToJson()`으로 JSON
  배열을 만들어 앱 캐시 디렉터리(`cacheDir/exports`)에 임시 파일로 쓰고,
  `Intent.ACTION_SEND` 공유하기 시트로 사용자가 원하는 곳(파일 앱, 클라우드,
  메신저 등)에 보낼 수 있게 한다. 가져오기는 `ActivityResultContracts.
  OpenDocument()`로 파일을 선택받아(MIME 타입은 일부 파일 관리자가 `.json`을
  `text/plain`으로 잘못 보고하는 경우가 있어 `*/*`로 넓게 허용) `Glossary.
  importFromJson()`으로 기존 용어집에 병합한다(지우지 않고 upsert). 개별
  항목이 형식에 안 맞으면 그 항목만 건너뛰고 계속 진행해, 파일 일부가
  깨져도 나머지는 정상적으로 가져와진다. `FileProvider`(업데이트 설치에
  이미 쓰던 것과 동일 authority)로 캐시 파일을 노출하려면 `file_paths.xml`에
  `cache-path` 선언이 필요해 추가했다(없으면 내보내기 시도 시
  `IllegalArgumentException`으로 크래시).

## 새 기능 (원문 대조 보기)

- **번역된 블록을 길게 누르면 원문을 Toast로 표시**: 번역이 이상하게 됐는지
  확인하고 싶어도 원문이 이미 화면에서 사라져 확인할 방법이 없었다.
  `tappApplyTranslations`가 텍스트를 치환하기 직전에 `data-tapp-original`
  속성으로 원문을 저장해두고, 500ms 롱프레스(터치/마우스 다운 이벤트 기반)를
  감지하면 `TranslateAppBridge.onShowOriginalText`로 Kotlin에 넘겨 Toast로
  보여준다. 링크/버튼 등 인터랙티브 요소가 있는 블록은 모바일 브라우저가
  롱프레스를 컨텍스트 메뉴로도 쓰므로 제외했다. 재시도(`retryBlock`) 성공 시
  같은 블록에 `tappApplyTranslations`가 다시 호출될 수 있어, 리스너가 중복
  등록되지 않도록 `data-tapp-longpress-bound` 플래그로 한 번만 붙도록 했다.

## 성능 개선 (자동 감지 모드 캐싱)

- **"자동 감지" 모드에서 언어 판별 결과를 캐싱**: 지금까지 자동 감지는 매번 새로
  언어를 감지+번역했다(같은 다국어 페이지를 재방문해도 캐시를 전혀 못 씀). 새
  테이블 `language_detection_cache`(원문 해시 → 감지된 언어 코드)를 추가해,
  같은 텍스트를 다시 만나면 언어 감지 API 호출(ML Kit 추론 또는 Cloud
  Translation의 감지 겸용 호출)을 건너뛰고 곧장 번역 캐시(`translation_cache`)까지
  재사용하도록 `PageTranslator.translateWithEffectiveLang`을 다시 짰다. 언어
  감지는 도착어와 무관하므로 도착어를 바꿔도 계속 재사용된다. 용어집이 적용된
  블록(플레이스홀더 치환)은 여전히 번역 결과 캐시를 쓰지 않지만(용어집 변경 시
  옛 치환 결과가 섞이는 문제 방지), 언어 감지 캐시는 그 경우에도 재사용해
  감지 비용만 아낀다(`useTranslationCache` 파라미터로 구분). 재시도
  (`retryBlock`)도 이전 실패 결과가 캐시로 남아 재시도가 무의미해지지 않도록
  번역 결과 캐시는 건너뛰고 감지 캐시만 재사용한다. DB 버전을 4→5로 올림
  (`fallbackToDestructiveMigration`이라 기존 캐시는 재구축됨).

## 배포 (전체 검수 중 발견 — 정식 릴리스 노트 누락 버그)

- **정식 태그의 릴리스 노트에서 베타 단계 커밋이 누락되던 문제**: `release.yml`의
  "직전 태그 찾기"(`git describe --tags --abbrev=0`)가 베타 태그까지 포함해서
  찾다 보니, `v36`을 정식 배포할 때 바로 앞의 `v36-beta2`를 "직전 태그"로 잡아
  버렸다. 그 결과 실제 v36 릴리스 노트가 "v36 정식 배포 문서화" 한 줄뿐이었고,
  베타 단계(v35 이후 v36-beta1/beta2)에 있었던 진짜 기능 변경("자동 감지" 추가,
  LLM 후처리 버그 수정) 커밋들이 통째로 빠져 있었다 — 앱 내 "업데이트 확인"
  다이얼로그가 이 releases body를 그대로 보여주므로 사용자가 실제 변경 내용을
  전혀 알 수 없는 상태였다. 정식 태그를 배포할 때는 다른 정식 태그만(`--match
  "v*[0-9]" --exclude '*-*'`), 베타 태그를 배포할 때는 기존처럼 아무 태그나
  기준으로 직전 태그를 찾도록 분기해서 고쳤다. (기존에 이미 올라간 v36 릴리스의
  body 자체는 GitHub 웹에서 수동으로 고치지 않는 한 그대로 남는다 — 다음
  정식 배포부터 정상 반영된다.)

## 새 기능 (출발어 자동 감지 — 다국어 혼합 페이지 지원, v36)

v36-beta1/beta2로 배포해 검증한 뒤 정식 반영. (아직 혼자 쓰는 단계라 베타
검증 단계 없이 바로 정식 배포로 확정했다.)
...
- **출발어 드롭다운에 "자동 감지" 옵션 추가**: 한 페이지 안에 여러 언어가 섞여
  ...
  - **(검수 중 발견/수정)** 자동 감지 모드에서 이미 도착어로 판별돼 번역하지
    않고 원문 그대로 둔 블록까지 LLM 후처리 대상(`postProcessTargets`)에 넣고
    있었다 — 원문=번역문이라 손댈 게 없는데도 "다듬어" 달라고 요청하는 셈이라,
    LLM이 엉뚱하게 변형할 위험만 있고 얻는 게 없어 감지된 언어가 도착어와
    같으면 후처리 대상에서 제외하도록 고쳤다(`PageTranslator.translateAndApply`).

## 버그 수정 (다크모드 언어 스피너 글자색)

- **다크모드에서 언어 선택 스피너 글자가 안 보이던 문제 해결**: 표준 리소스
  `android.R.layout.simple_spinner_dropdown_item`이 라이트 테마 기준 어두운
  글자색을 고정으로 써서, 다크모드에서 어두운 배경에 어두운 글자가 겹쳐 거의
  안 보이는 문제가 있었다(v19에서 커스텀 레이아웃으로 고치려다 드롭다운 자체가
  안 열리는 회귀가 발생해 v22에서 롤백했던 그 문제). 이번엔 레이아웃 리소스는
  그대로 두고 `ArrayAdapter.getView`/`getDropDownView`가 반환한 뷰의 텍스트
  색만 `textColorPrimary`로 런타임에 덮어쓰는 방식으로 재시도했고,
  `v34-beta1` pre-release로 먼저 배포해 실기기(다크모드)에서 드롭다운이
  정상적으로 열리고 글자도 잘 보이는 것을 확인한 뒤 정식 반영했다.
